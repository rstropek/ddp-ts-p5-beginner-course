...
Part 7 links a third kind of activity, a **coding** activity. It is not a chat: it is an
OpenAI-compatible endpoint that an outside coding agent (pi.dev, OpenCode, the playground's
own chat panel) talks to, with the course's coding guidelines injected server-side. The
prompt lives in `ddp-coding-buddy.yaml` in the repo root, and the `coding` shortcode links
it by registry key like the others:
...
school account, and that page mints them a personal `nvk-…` key, stable across visits and
devices, which they paste into their coding agent. That is why the code can be printed in
the book at all, and why the shortcode's fixed body text says the code alone opens nothing:
a code pasted into a coding agent earns an opaque 401 with no hint.

Two consequences the prose has to carry, so they are the box's fixed body text in
`_extensions/coding/coding.lua` rather than repeated per chapter:

* **Requesting a key is attributed.** The student's name goes on a list the teacher sees.
  Quizzes are anonymous and this is not, which students are told before they click. The
  conversations themselves are never stored.
* **Access is the availability window.** There is no per-student revocation. Closing a
...
**Codes come from the registry, not from hand-minting**, and quizzes, tutors, and the
coding activity go through the same one. Two files in the repo root hold the whole mapping:

| File | What |
| --- | --- |
| `ddp-activities.yaml` | Hand-written. Every activity under a key (`first-program`, `tutor-conditions`), grouped into `tutors:`, `quizzes:`, and `coding:`, with its file path and any minting options. This is the file you edit. |
...
document metadata and the `quiz`, `tutor`, and `coding` shortcodes resolve the key at
render time. Nothing is fetched during a render, and a key missing from the lock file
**fails the build** instead of publishing a dead link.

Publishing a new activity is therefore: write the YAML, validate it, push, add one entry to
...
| `_extensions/` | The Quarto extensions providing the `example`, `playground`, `quiz`, `tutor`, and `coding` shortcodes |
...
The `example`, `quiz`, `tutor`, and `coding` shortcodes all render for paper as well as
screen: in the PDF, where a button is dead ink, the callout also carries a QR code and the
printed address. That printed-link block is duplicated in the four Lua files, so a change to
one belongs in all four.
