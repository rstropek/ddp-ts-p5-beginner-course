...
third callback next to `setup` and `draw`. p5.js calls `keyPressed`
once for every key the player presses:
...
Inside `keyPressed`, the global variable `key` holds the character
that was pressed, as a string. Pressing the A key makes `key` the
string `"a"`, and holding shift makes it `"A"`. This first part of
Melting Snowman ignores which key it was and only counts the
presses, so every key melts the snowman a bit further. The letters
start to matter when the guessing logic arrives.

The pairing of `keyPressed` and `redraw()` is the whole event loop
of this program. A key changes a variable, `redraw()` repaints the
