...
An image is not part of your code. The picture file lives somewhere on
the internet, under an address, and your program has to fetch it before
it can draw it. Fetching takes time, a few milliseconds on a good
connection and much longer on a bad one, so p5 gives you a command that
waits for the picture and hands it over when it has arrived.
