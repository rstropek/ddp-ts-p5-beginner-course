...
Two ideas meet in this chapter: a loop and an event. If your rays
don't show up, smear across the canvas, or wear the wrong colors,
tell the tutor which of the chapter's two exercises you are in and
show it your loop.
...
any other: it may live in any function. When a loop lives in
`mouseMoved` (@sec-mouse-moved), it runs again on every mouse move,
drawing its whole picture fresh each time. That is exactly how the
rays work. Here is the plan of such a program, with the loop itself
still missing:
...
loop visited. The new idea is that in each round, the loop variable
`i` marks four points at once, one per edge:
...
works, yellow before the left-and-right pair and lime before the
...
One more difference from the grid loop is easy to miss: the condition.
...
1. **Make a loop table** for `SIZE = 100` and `MARGIN = 25`, with the
   condition `i <= SIZE - MARGIN`. Which values does `i` take, and
   how many rounds is that? Does the last anchor position,
   `SIZE - MARGIN`, get through?
2. **Read the starter code.** It is the plan from the beginning of
   the chapter (@sec-loop-in-event), ready to go: the constants
   `SIZE` and `MARGIN` are declared, and `mouseMoved` already
   repaints the background and sets the stroke weight. A comment
   marks the spot where your loop belongs.
3. **Start with one edge.** Set `stroke("yellow")` before the loop
   for now, and write the loop with a single statement:
   `line(MARGIN, i, mouseX, mouseY);`. Run it and move the mouse.
   One fan of yellow rays from the left edge, nothing else.
4. **Add the other three edges**, one `line` statement per edge, and
   move the colors into the loop body: yellow before the
   left-and-right pair, lime before the top-and-bottom pair.
...
gradient. If the update ran *after* the loop instead, every ray in
one frame would get the same color, and the fan would change color
as a whole, one hue per frame. Both are valid designs. The exercise
builds the first one, and trying the second for a moment is a great
experiment.
:::
