...
restore it. Combined with loops, they open a new way of drawing:
instead of computing every position, you draw in one place and move
the paper under the pen. Both exercises are discovery scripts, in
which you uncomment code line by line, predict, and watch.

## AI tutor {#sec-tutor-move-origin}

Moving the origin confuses everyone at first, because the code names
...
## The origin moves {#sec-translate-relative}
...
![translate moves the origin: after translate(100, 100), the point (0, 0) sits 100 pixels right and down from the corner, and circle(0, 0, 50) is centered exactly on that new origin.](images/translate-origin.svg){width=340}

Three facts complete the picture, and the first exercise will let
you discover each one:
...
Now combine `translate` with a loop. The goal picture at the top of
the chapter shows a diagonal of circles, and the rule is that you
draw every circle with the very same command,
`circle(0, 0, CIRCLE_DIAMETER)`, no other coordinates allowed.
`SIZE` is the canvas size and `CIRCLE_DIAMETER` the diameter of one
circle, both constants:

```typescript
circle(0, 0, CIRCLE_DIAMETER);
for (let i: number = 0; i <= SIZE; i += CIRCLE_DIAMETER) {
...
computing `circle(i, i, ...)`. Same picture, two ways of thinking:
move the pen, or move the paper. For a single diagonal, both are
fine. For the patterns of the next chapter, moving the paper wins, and
you'll see why.
...
usually you want something gentler, a "remember this spot, let me
wander, then bring me back". That is exactly what `push` and `pop`
do:

- `push()` writes a note: it saves the current origin **and** the
  current style settings (`fill`, `stroke`, `strokeWeight`, and the
  other style commands).
- `pop()` takes the most recent note and restores everything on it.

One concrete timeline shows the note-taking at work:

| statement | origin afterward |
|--------------------|-------------------|
...
: A push and pop timeline, with the origin after every statement. {#tbl-push-pop}
...
2. **Task 2: write the loop.** The goal is a diagonal of circles:
   diameter 50, the first centered at the corner (0, 0), each next
   one 50 right and 50 down. One circle at (0, 0) before the loop,
   then a for loop whose body is exactly two statements, the
   translate and the circle, as in the section above
   (@sec-cumulative). No computed coordinates allowed.
3. **Experiment.** Change the translate to `(CIRCLE_DIAMETER, 0)`
