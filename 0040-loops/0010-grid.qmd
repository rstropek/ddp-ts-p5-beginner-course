...
Look at the drawing above and count what it takes. It needs 15
vertical and 15 horizontal lines, 30 `line` statements that differ
in exactly one number. You could type them all. You typed six `circle` calls for
...
nothing can be confusing. The tutor of this part knows everything
about the while loop. Show it your loop and tell it what you
expected and what you got.

{{< tutor tutor-loops >}}

Watch out for one danger before you write your very first loop. A
...
"line at 25, line at 50, line at 75". And the worst part comes
later. If you want a finer grid with a line every 20 pixels, you
have to rewrite every single statement.
...
::: {.callout-important}
## Course rule: the loop variable gets a data type too
`let i: number = GRID;` declares a variable like every other
declaration, so it carries its data type. The rule from the
Variables part (@sec-data-types) has no exception for loops, and
that stays true for every loop shape in this book. Code you find on
the web often leaves the type out in a loop, because TypeScript can
work it out on its own. In this course you write it, every time.
:::
...
good. Meaningless names like `temp`, `helper`, `thing`, or `stuff`
stay forbidden for everything else, because the naming rules from
the Variables part (@sec-const-naming) still apply.
:::
...
visible on paper before the browser ever sees the code. The
following chapters won't repeat this instruction every time; the
technique is yours now, and it helps with every loop you write from
now on.
:::
...
Play computer: `i` is 25, the check says `true`, a line appears, and
`i` is... still 25. Check `true`, line, still 25. The condition can
never become `false`, so the loop never ends. This is the endless
loop from the start of the chapter (@sec-tutor-grid), and now you
can see exactly how one happens. The classic cause is a forgotten
update.
...
The second exercise starts where the grid ends, with both grid
loops already in the starter code. On top of the grid you draw colored
...
2. **Make a loop table** for the yellow loop with
