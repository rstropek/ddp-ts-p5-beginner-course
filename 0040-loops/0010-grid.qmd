...
good. What stays forbidden are meaningless names for everything else:
the naming rules from the Variables part (@sec-const-naming) still
apply.
:::
...
never become `false`, so the loop never ends. Programmers call this
an **infinite loop**, and it is the classic loop mistake.

::: {.callout-warning}
## The frozen tab
An infinite loop shows no error message and no red squiggle: the code
