...
purpose: the exercises after it need one more drawing tool. Back in
the practice projects of part 1, the soccer field recipe contained
three magic words: `push`, `translate`, and `scale`. The Loops part
explained the first two. `scale` is the last one, and after this
chapter the recipe holds no secrets: `scale` makes everything you
draw bigger or smaller by a factor, and, as the goal picture shows,
it does something to positions that surprises almost everyone.
...
`scale(2)` comes out 6 pixels wide. The sample code compensates by
setting `strokeWeight(3 / 2)` after `scale(2)`: one and a half
pixels, doubled by the scaling, gives the 3 pixels the eye expects.
Factors below 1 work as well: `scale(0.5)` shrinks everything to
half size, and decimals like `scale(2.5)` are fine.
...
1. **Paper first.** The biggest circle has diameter 50 times 4,
   which is 200 pixels. Mark a center on your sketch of the 400 by
   400 canvas so the whole ring fits; the sample uses (150, 150),
   but any spot at least 100 pixels from every edge works.
