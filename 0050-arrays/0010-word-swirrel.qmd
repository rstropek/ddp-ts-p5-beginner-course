...
the string. A string is itself a list, a row of characters, and in
this chapter you take one apart and put it back together in a new
order. The result is a little game: Word Swirrel scrambles a secret
word, and the player has to guess what it was.

## AI tutor {#sec-tutor-word-swirrel}

Indexes are new in this chapter, and the fact that counting starts
at 0 trips up everyone once. If a letter comes out that you did not
expect, or `undefined` shows up on your canvas, tell the tutor what
you wrote and what appeared.
...
The index answers the question "how many characters come before this
one?" Before the first character there are zero, so its index is 0.
Programmers count like this everywhere, and from now on you will
too: the fifth character of a string has the index 4.
:::
...
`` `Clicks: ${clickCount}` ``. This chapter adds the second way:
gluing strings together with `+`, and growing a variable with `+=`,
...
In the Variables part, the course rule said: strings grow only
...
If you leave the second number away, `substring(start)` copies
...
One thing `substring` never does: it never changes the string it is
called on. It hands you a copy of the piece, and the original stays
as it is. To keep the shorter word, you assign the result back to
the variable, as in the line above.

## The scramble recipe {#sec-scramble-recipe}

Now the pieces click together into the heart of Word Swirrel. The
idea is drawing letters out of a bag: pick a random letter of the
word, append it to the scrambled result, remove it from the bag, and
...
Initialize: it starts as a copy of the secret word. Check: as long
as characters are left. Do the work: move one random character over.
Update: the substring line makes the bag one character shorter, so
`length` sinks by one each round and the loop is guaranteed to end.
...
the playground calls it for you when the button is clicked. New is
the part between the parentheses: `textInput` is a **parameter**, a
variable that the playground fills with the typed text before your
code runs. Inside the braces you use `textInput` like any string
variable. Writing your own functions with parameters comes in a
later part of the book; here the signature is given, and your job is
only the body:
...
Comparing strings with `===` you know from rock, paper, scissors,
...
1. **Type in the program.** Use the listings above: the scramble
   loop goes into `setup`, after it draw `scrambledWord` centered on
   the canvas with `text`, and the comparison goes into `guess`.
   Type, don't paste: your fingers learn the index and substring
   syntax now, before the array chapters need it.
...
4. **Read the sample solution.** It is the same program with long
   explaining comments, including a picture of how the two
