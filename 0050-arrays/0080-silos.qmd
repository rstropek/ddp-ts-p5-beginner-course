...
factory wants a dashboard for its silos. It should show every fill
level, warn in red when one runs critical, and let the operator add
or remove material with buttons. Every idea of this part comes
together here: the fill levels arrive as a string and are parsed into
an array, the array decides how many silos exist, the mouse *changes*
...
programs fail in small places: one wrong constant in a comparison,
...
this in the parsing chapter, with one difference; back then, the
program drew every parsed value immediately and threw it away. Now
the values must be *kept*, and where do parsed values live? In an
array:
...
The digits grow the number differently this time, with arithmetic
instead of a buffer string. `num * 10 + digit` shifts the number one
...
moment the array is in charge of everything in the program you are
about to build:
...
The drawing itself follows the bubbles rule: `draw` wipes the
...
That split, where events write the state and `draw` paints the
state, is how real user interfaces work, from this dashboard up to
...
for (let i: number = 0; i < silos.length; i++) {
    const x: number = SILO_GAP + i * (SILO_WIDTH + SILO_GAP);
    silosX.push(x);
    upX.push(x);
...
its button). The last new tool of the part is the bare `return;`,
which ends `mouseClicked` on the spot after a hit. One click means
one button; once it is found and handled, there is nothing left to
check, and the `return` says exactly that.

::: {.callout-note}
## The mouse ignores translate
If your `draw` translates to each silo before drawing it, remember
that `mouseX` and `mouseY` always measure from the canvas corner,
untouched by any translate. Hit tests must therefore compare
against the stored canvas positions `upX[i]` and `BUTTON_TOP`, not
against (0, 0).
...
2. **Task 2: the silos.** Sketch one silo first, with the
   constants labeled (`SILOS_TOP`, `SILO_WIDTH`, `SILO_HEIGHT`)
   and the bar growing from the bottom; a labeled sketch turns the
   drawing code into simple reading. Then draw the outline (two
...
3. **Task 3: the buttons.** Two yellow squares above each silo,
   ...
   button, as around every other drawing block.
