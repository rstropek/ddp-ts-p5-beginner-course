...
![An array of strings: one variable that holds a whole row of values. Every value in the row has an index, and the counting starts at 0, exactly like in a string.](images/array-of-strings.svg){width=420}
...
array, show the tutor your lines; the tutor finds a lost comma or
bracket in seconds.
...
Read it in three parts. The type annotation `string[]` says that
this variable holds an *array of strings*, a row of values where
each one is a string. The square brackets on the right are the
**array literal**: the elements, separated by commas, between `[`
and `]`. Writing each element on its own line is not required, but
for long lists it keeps the code readable, and you may leave a
comma after the last element too. The name is in ALL_CAPS because
the word list is configuration, like `SIZE` and `GRID` were, so the
naming rules don't change.
...
its value at the declaration. Level 3 therefore switches the
keyword to `let`, and you will find that change already made when
...
4. **Experiment.** Add your own words, short and long; the letter
   ...
   change. Then shrink `WORDS` to a single word and run twice,
   because with one element, `random` has no choice.
