...
teaches the opposite move: `splice` removes a bubble whose time is
up. Along the way you meet `millis`, the program's clock, and the
...
the tutor what you see and paste your push or splice lines.
...
`circlesDiameter`, `circlesFill`. Creating one bubble means one
push into *each* of them:
...
frame, `draw` checks the clock against the deadline. Not due yet:
do nothing. Due: create the bubble and set a new deadline half a
second to two seconds into the future. The wall clock keeps
running; the program just compares against it. You will use this
deadline pattern for every "every so often" behavior from now on.

## The arrays are the truth {#sec-arrays-are-truth}

Here is the whole `draw`, and its shape matters more than any line:
...
Compare that with the colored rays back in the Loops part: there,
...
picture follows. The next section does exactly that.

## Your exercise: Bubbles {#sec-bubbles-exercise}

The exercise is a type-in of the program above; the code is in the
exercise description as a picture, so your fingers type every
bracket themselves.
...
array, filled at creation with its personal deadline, the same
pattern as `nextCircle`:
...
down to 0. That is not a stylistic whim. When `splice` removes
