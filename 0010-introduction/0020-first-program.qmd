...
By the end of this chapter you'll have written your very first real program and watched a small arrangement of shapes appear on the canvas. You'll get there by typing the code exactly as it's given, one character at a time, and seeing for yourself how a program comes to life. It's a small result, but it's entirely yours. This is the moment programming stops being an idea and starts being something you do.
...
first and paint the background afterward, the pink paint covers your
...
## Your statements live in the setup function {#sec-setup}

Your statements don't stand alone in the file. They sit inside a **function**,
a named block of statements that runs as a unit. This one is named `setup`,
and the computer runs it once when your program starts. Your first exercise
has this shape, with a few more statements in the middle:
...
The line `function setup() {` opens the block, the closing `}` at the very
bottom ends it, and everything between them is your program. The statements
inside are shifted to the right so you can see at a glance what belongs to
`setup`.

For now, one rule is enough. Every statement you write in this part goes
inside `setup`, between the curly braces. Later in the course you'll write
functions with other names, and functions that run at other moments.

## Red squiggly lines show you your mistakes {#sec-squiggles}

While you type, the editor checks your code. When something is wrong, a
mistyped name, a missing parenthesis, a lost curly brace, it underlines the
problem with a **red squiggly line**, just like a spell checker underlines a
typo in a text document.
...
much easier to read, and mistakes like a missing curly brace are easier to
find.
